...
def index(request):
    ...
    return render(request, 's6app/index.html')


def about(request):
    logger.info(f'Посещение страницы {__name__}: about')
    return render(request, 's6app/about.html')
...
def orders(request, customer_id):
    ...
    return render(request, 's6app/orders.html', context)
...
def stats(request, customer_id, scope_in_days=365):
    ...
    return render(request, 's6app/stats.html', context)